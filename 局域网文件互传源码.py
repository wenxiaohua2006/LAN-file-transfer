...
def CacheGet():
    ...
    def get_cache():
        global Cache
        Cache = build_folder_structure(config["FileCataloguePath"])
    if thre.is_alive():
        return -1
    else:
...
config = {"SingleUpload":False,"download":True,"write":True,'dirwrite':True,"FileMax":1024,"sysMax":1,"MaxSpeed":1024,"SysCataloguePath":"download","FileCataloguePath":"","log":{"upload":[],"download":[],"error":[]},"type":["zip","txt"],"IPsecure":["192.168.0.5"],"typeinspect":False,'download_Dir':6}
def start(filepath):
    if filepath == "":
        ...
        if os.path.exists(filepath):
            if os.path.isdir(filepath):
                ...
                if filelen > 2000:
                    mode = messagebox.askyesno(title="确认", message=f"当前目录文件数量{filelen}超过2400,数量过多可能导致加载过慢\n你需要重新选择目录吗?")
                    if mode:
                        return False
...
def upload_zip():
    ...
    if ip_address in config["IPsecure"]:
        return render_template("end.html",title="不好意思,服务器已将你列入黑名单,你无权限访问")
...
def set_config():
    ...
    filepathif = jsondata.get('FileCataloguePath').replace(' ','')
    dirpathif = jsondata.get('SysCataloguePath').replace(' ','')
    if filepathif != "":
        if not os.path.exists(filepathif):
            return jsonify({"code":"共享目录不存在,请重新选择"})
    if dirpathif:
        if not os.path.exists(dirpathif):
            if is_valid_path(filepathif):
                pass
            else:
